...
class UserAgentTool
{
    ...
    protected function assignProperties(Tool\Device $device)
    {
        ...
        if (strpos($device->device_ua, 'iPhone') !== false || strpos($device->device_ua, 'iPad') !== false || strpos($device->device_ua, 'iPod') !== false) {
            ...
            if ($device->browser->setRegex($device->browser_ua, '/^Mozilla\/[45]\.0.+?like Mac OS X.+?CriOS\/([0-9]+?)\.[0-9].+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+\./', 'Chrome Mobile iOS', 1)) {
                return $device;
            }

            //Is UA UC Web Browser?
            if ($device->browser->setRegex($device->browser_ua, '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+.*UCBrowser\/?([0-9]+)\./', 'UC Web Browser on iOS', 1)) {
                return $device;
            }

            //Is UA Mobile iOS Safari?
            if ($device->browser->setRegex($device->browser_ua, '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+.*FBAN/', 'FaceBook app on iPhone', $device->os->version)) {
                return $device;
            }
        ...
        if (strpos($device->device_ua, 'OviBrowser') !== false && $device->browser->setRegex($device->browser_ua, '/\bS40OviBrowser\/([0-9]\.[0-9])/', 'S40 Ovi Browser', 1)
        ...
        //Is UA Blackberry?
        if (strpos($device->device_ua, 'BlackBerry') !== false
            && $device->os->setRegex($device->device_ua, '/(?:BlackBerry)|(?:^Mozilla\/5.0 \(BB10; ([a-zA-Z0-9])\))/', 'BlackBerry')
        ) {
        ...
        if (strpos($device->device_ua, 'NetFront') !== false
            && $device->browser->setRegex($device->browser_ua, '/NetFront\/([0-9]\.[0-9])/', 'NetFront', 1)
        ) {
            return $device;
        }
        ...
        if (strpos($device->device_ua, 'Dolfin') !== false
            && $device->browser->setRegex($device->browser_ua, '/UP\.(?:Browser|Link)\/([0-9]\.[0-9])/', 'Openwave Browser', 1)
        ) {
            return $device;
        }
