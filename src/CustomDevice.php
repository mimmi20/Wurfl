 ...
 * @property-read Device\ModelDeviceInterface[] $modelDevices
 * @property-read string                               $pointing_method
 * @property-read string                               $is_tablet
 * @property-read bool                                 $can_assign_phone_number
 * @property-read string                               $device_os
 * @property-read string                               $ux_full_desktop
 * @property-read string                               $preferred_markup
 */
class CustomDevice implements CustomDeviceInterface
{
    /**
     * @var \Wurfl\Device\ModelDeviceInterface[]
     */
    private $modelDevices = array();
        ...
        foreach ($modelDevices as $modelDevice) {
            if (!$modelDevice instanceof Device\ModelDeviceInterface) {
                throw new \InvalidArgumentException(
                    'each ModelDevice has to implement the \Wurfl\Device\ModelDeviceInterface'
                );
            }
     ...
     *
     * @return Device\ModelDeviceInterface[]
     */
    ...
    public function getVirtualCapabilityProvider()
    {
        if (null === $this->virtualCapabilityProvider) {
            ...
            $this->setVirtualCapabilityProvider(new VirtualCapabilityProvider($device, $request));
        }
